...
#right_eye:81-90-89-87-91-93-96-94-95
#mouth:52-55-56-53-59-58-69-68-67-71

#frame=cv2.imread('input/paris.jpg')
#image=cv2.resize(image,(0,0),fx=0.5,fy=0.5)
        ...
        for pred in fa.get_landmarks(frame, boxes):#for loop for any person in image
            ...
            landmarks_right_eye=[]
            for i in[81,90,89,87,91,93,96,94,95]:
                landmarks_right_eye.append(tuple(pred_int[i]))

            landmarks_mouth=[]
            for i in[52,55,59,58,69,68,67,71,63,64]:
                landmarks_mouth.append(tuple(pred_int[i]))    


            #landmarks_left_eye=np.array([[tuple(pred_int[i]) for i in range(len(pred_int)) if i in[35,36,33,37,39,42,40,41]]])
            #landmarks_right_eye=[tuple(pred_int[i]) for i in range(len(pred_int)) if i in[81,90,89,87,91,93,96,94,95]]
            #landmarks_mouth=[tuple(pred_int[i]) for i in range(len(pred_int)) if i in[52,55,56,53,59,58,69,68,67,71]]

            landmarks_left_eye=np.array([landmarks_left_eye])
            landmarks_right_eye=np.array([landmarks_right_eye])
            landmarks_mouth=np.array([landmarks_mouth])

            frame=zoom_effect(frame,landmarks_left_eye)
            #print('lefteye_landmark',landmarks_left_eye)
            frame=zoom_effect(frame,landmarks_right_eye)
            frame=zoom_effect(frame,landmarks_mouth)



            # cv2.drawContours(mask,[landmarks_left_eye],-1,(255,255,255),-1)
            # cv2.drawContours(mask,[landmarks_right_eye],-1,(255,255,255),-1)
            # cv2.drawContours(mask,[landmarks_mouth],-1,(255,255,255),-1)
            # for index,p in enumerate(landmarks_left_eye,landmarks_right_eye,landmarks_mouth):
            #     #p=coordinate of landmark
